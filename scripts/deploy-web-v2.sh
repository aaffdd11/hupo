...
# ★★ 2026-09-24：**字体文件名也要带内容指纹**（在预压缩之前）
#   主人手机上的"字都在、图标全没有"就是这条：字体 URL 不带指纹 ⇒ 浏览器里那份旧的/坏的一直被 304 续用，
#   而图标字体是按这一版用到的图标 tree-shake 出来的**子集** ⇒ 旧子集里没有新图标 ⇒ 图标全空白。
"$NODE_BIN" "$ROOT/scripts/fingerprint-fonts.mjs" "$WEB" | sed 's/^/  /' || {
  echo "  ✗ 字体指纹失败"; exit 1; }
