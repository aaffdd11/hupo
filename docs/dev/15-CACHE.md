...
## 补 · 🔴 **"整页空白、一个图标都没有"的根因**（2026-09-24）

主人报：*"很奇怪，依然没有图标，是空白的"*。查下来是**缓存那件事的另一半**：

| 环节 | 事实 |
|---|---|
| 发布 | `deploy-web-v2.sh` 里是 `rm -rf "$WEB"` —— **整目录删掉重来** ⇒ 上一版的 `main.<旧指纹>.dart.js` **不存在了** |
| 老访客 | 他浏览器里可能还存着**上一版的 `index.html`**（它引用那个旧指纹） |
| 服务端 | 未知路径**回 `index.html`**（SPA 兜底）⇒ 浏览器把**一段 HTML 当 JS 解析** ⇒ **白屏** |
| 实测 | `curl -o /dev/null -w '%{http_code}' https://w.stalkerai.cn/main.deadbeef0000.dart.js` → **200**（不是 404）——就是这条兜底在"装作有那个文件" |

**修法**：发布时把**最近 3 份**入口文件留着（`main.*.dart.js` / `flutter_bootstrap.*.js`），
更老的才删。它们很小，而同指纹必然同内容 ⇒ 老访客拿到的还是**真 JS**，页面照常起来。

**验证**（当场做的）：往 `web/` 里放一份假的旧入口 `main.000000000000.dart.js` ⇒ 跑一次发布
⇒ 它**还在**（"手上留着的入口：2 份"）⇒ 清掉假的收工。

⚠️ **留给下回的一件事**（没做）：那条兜底对"看起来像文件的路径"（`.js` / `.css` / `.wasm` / `.otf`）
**也该回 404** —— 拿 HTML 冒充 JS，是把"缺文件"变成"白屏"，把病因藏起来了。
现在靠"留着旧入口"绕过去，但**下一个不认识的文件仍会白屏**。

⚠️ **给用户的自救**（这次主人就是这么碰上的）：**强制刷新**或**换个隐私窗口**打开 ——
他手里那一页是**旧 HTML**，只有重新取一次 HTML 才会指向当前这一版。

---

## 补2 · **字体文件的 URL 也上指纹**（2026-09-24 · 主人：*"字都在，只有图标不在"*）

同一个病根的**第二处**：入口文件有指纹，**字体没有**。

| 环节 | 事实 |
|---|---|
| 图标字体 | `assets/fonts/MaterialIcons-Regular.otf` —— **URL 里没有指纹**，而它每版**内容都变**（Flutter 按这一版用到的图标 **tree-shake** 出子集） |
| 缓存 | 它带 `cache-control: no-cache`，但那只让浏览器**拿 304 继续用手里那份** ⇒ 旧/坏的子集**永远粘住** |
| 症状 | **汉字全在、图标全空白**（字与图标是两个字体文件）——正是主人报的那一条 |
| 我这边的反证 | 同一份字体我这边是好的（`OTTO` + 字形都在、`19880` 字节）⇒ 问题只在**客户端那份缓存** |

**修法**：`scripts/fingerprint-fonts.mjs`（发布时跑，**在预压缩之前**）：
按内容哈希算一份 `MaterialIcons-Regular.<hash8>.otf`，把 `fontManifest.json` 指过去
⇒ **内容一变 URL 就变**，浏览器只能重新取。

⚠️ **两个坑（都是当场被抓出来的）**：
1. **原文件名那份必须留着**：`assets/AssetManifest.json` 与 `assets/AssetManifest.bin`
   （Flutter 自己的二进制清单）里也列着字体，而它们**不由我们改写** ⇒ 只**改名**的话那两处会 404
   —— **部署脚本自己的引用检查当场拦下**（`✗ assets/…/CupertinoIcons.ttf ← AssetManifest.json 找不到`，
   于是"没有部署"）。⇒ 改成**复制**一份带指纹的，原名那份不动。
2. 跑的位置必须是**预压缩之前**，否则 `.br`/`.gz` 还是按旧名字生成的。
