...
> ⚠️ **2026-09-26 补（后续 `124-TOUCH-REGRESSION.md`）**：主人再报一次「聊天依然收起才能发送」，
> 顺着**触屏**那条路查下去，**找到了**一个能一次解释三条症状的根因：
> **手按住气泡 ≥500ms（`kLongPressTimeout`）⇒ 弹一层 `showModalBottomSheet`
> ⇒ 它那层 barrier 铺满全屏 ⇒ 时间线、抓手行按钮、输入条一起失效**；
> 而**收起的窗口里没有气泡可长按** ⇒ 只有它正常 —— 就是「收起才能发送」。
> 本文件 §五 里"最可能的方向"两条（手机不报键盘高度 / 语音卡住）**都不是它**；
> 这一节留下的是**当时的读数**与**当时为什么看不见**（用的都是合成瞬点，到不了 500ms）。
> 详细读数、修法与**没结的那条账**一律看 [`124`](124-TOUCH-REGRESSION.md)。
