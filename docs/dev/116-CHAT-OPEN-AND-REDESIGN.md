...
## 结项（2026-09-26 · 目标「聊天窗口重做」的账）

主人 2026-09-26 定的形状（*"首先全部开放，聊天窗口的设计也要重做。"* ＋ 当场签的"桌面留、浮窗留、继续 Flutter"）**已经落地**。逐批的账：

| 批 | 是什么 | 判据 | 真机读数 | 部署 |
|---|---|---|---|---|
| 1 | 设计骨架（DSH token：亮/暗 · 0.5px 描边 · 字号轴）＋ **工具行 / 系统提示词行 / 每轮用量行 / 过程折叠与计数**；服务端 `tool/call`·`tool/result`·`turn/usage`·`system/prompt` 四帧持久事件 | 服务端 ＋6 条 · 客户端 38 条（含反例）；`D1.1·补`＝聊天窗口内放开内部词 | 临时核心＋真 agent＋真浏览器：四帧落盘/重放，浏览器认出 `tool/call`、`turn/usage` | 产品层 `cd782714a751 → 16f4f466e58a`；入口 `d7848841b115` |
| 2 | **排队看得见、撤得掉**（`queue/changed` ＋ `unsay`） | Q1–Q7（各带反例）＋ 客户端 10＋7＋a11y 25 | 挂着的一轮里发第二句 ⇒ 收到 `{"items":[{"messageId":"m_117_b",…}],"count":1}`；撤掉 ⇒ `count:0`；垃圾帧不打崩；盘上 0 行（瞬态） | 产品层 `16f4f466e58a → a6b0f6c279d7`；入口 `7f2fb80ad227` |
| 3 | **轨迹那一屏**（聊天/轨迹两档 · 一屏表 · 只显示真收到的） | unit 31 ＋ widget 6（含"切回来滚动位置一点没动"）＋ a11y ＋20 | 构建产物里 `\u8f6e`/`\u8f68\u8ff9`（轮/轨迹）确在 | 入口 `3c13ae2f9edf` |
| 4 | **外观（亮/暗/跟随系统）＋ 字号轴 12–17**（浮窗内部） | unit 33 ＋ widget 23 ＋ a11y ＋18（用户字号 12/14/17 × 五档） | 产物里 `\u6697`/`\u8ddf\u968f\u7cfb\u7edf`/`\u8fd9\u5757\u7a97\u53e3` 各 1 处 | 入口 `8e6010370fa5` |
| 4·补 | 过程行（步骤流水/推理原文）的**字号也接上**用户轴（14 档逐像素不变） | unit 735 · a11y 382 | 服务端一个字没动 | 入口 `cb3c68fb118f` |
| 5 | **右栏**＝"这一窗动过哪些文件"（够宽挤、不够宽盖；点开看那一次调用的入参；截断如实说） | unit 33 ＋ widget 21 ＋ a11y ＋13 | 产物里「这一窗动过」1 处、「文件」3 处 | 入口 `3de6abfcf3e1` |

**按证据砍掉的（不是"以后再说"）**：

- **插话（Steer）**：我们 L2↔L3 是 SDK stdio，`dsh-sdk-jsonrpc-server` 那侧**只有 `session/prompt`**（本机 grep 实测）⇒ 没有 inbox/steer 入口；DSH 的窗口能插话是因为它连 web 档那套。要插话＝换 L2↔L3（架构级）。见 `117` 附二。
- **改一条排队的话（Edit）**：投递那一刻 `user/echo` 已落进可见日志 ⇒ 改票不改屏 ＝ 页面说假话。替代：撤掉重打。见 `117` 附二。
- 文档预览（要读盘/新接口）· 右栏 tab 带 · 拖宽 · 记住右栏开着（都是 `120` §五 明说砍的）。

**还欠的 / 卡在主人身上的**：

1. **屏幕上的那一张图**：四次尝试都展不开浮窗（`118` 附节记了负结果）⇒ 要"启动即展开"的调试开关（**要主人一句话**）。
2. **整机暗色**：今天只有聊天窗口跟外观走；整机暗＝给暖白纸＋朱红**发明**一份暗色版（品牌决定，**要主人一句话**）。
3. 右栏**记住"开着"** / **拖宽**（要就加）。

⇒ **本目标（主人签的那一版形状）到此收口**；上面三条是**新的、要主人拍板才动**的事。
