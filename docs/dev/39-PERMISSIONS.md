...
镜像是 `scratch + node`（**当时没有 shell** —— ⚠️ **2026-09-25 起不是了**：主人拍板「甲」，
镜像补了 `/bin/bash` ＋ 静态 busybox 那批 applet，见 `77-BLOCKERS.md` B18 与 `#133`）。
下面这些探针**当时**是用 `/bin/node /mnt/xxx.mjs` 挂进去跑的 —— 那种写法**现在照样能用**
（而且更省事：`podman exec … /bin/bash -c …` 也行）。
...
**判据 1**（在**真容器**里跑的，不是读代码推的 —— 两个容器共享一个卷，`--user` 换身份；⚠️ **当时**镜像是 `scratch + node`，**没有 shell** —— 那天探针只能走 `/bin/node`；**2026-09-25 起镜像里有 shell 了**，见 B18 / `#133`）：
...
| 2 | ✅ **客户端缓存按 `sub` 分命名空间 + 退出清全部** —— **2026-09-21 做完了**（`token_sub.dart` + 两个 store + 控制器；9 条判据 + **四处变异验证** + 已部署公网 `6926dc6d2819`） | ⚠️ 只剩「注销撤全部令牌」那一步（`Auth` 今天做不到，见 `38` §8.4） |
| 3 | 「新号进来，容器还没好」那个等待屏 | 要容器状态接口 |
| 4 | 🟡 **`dsh` 进镜像 ✅ 做完了**（②-1，2026-09-21：镜像 130→385MB，容器里 `dsh --version` = `0.1.5-rc.1`、空 `DSH_HOME` 能起 sdk、人格层挂得上、服务照常 200 —— 见 `34-CONTAINER.md` §七） · **换手到 uid 1000 ✅ 做完了**（②-2：`HUPO_AGENT_UID/GID` + spawn 降权 + `/data/dsh` 归 agent + 横幅如实报身份；7 条容器判据含一对正负对照，见 `34-CONTAINER.md` §八） · **root 小代理持 key ✅ 做完了**（②-3：`src/model-proxy.mjs` + 静态 patch 改 `baseURL`/`apiKeyEnv`；容器里实测：agent 经代理 **200**、自己读 key 文件 **`EACCES`**、上游收到的是**真 key 而不是占位符**、V4b 命中 **0**；9 条单测 + **四处变异验证**，见 `34-CONTAINER.md` §九） · ✅ **key 从外面注入也做完了**（②-4：宿主一租户一个套接字 + 容器**只挂自己那一个** + 容器开机连出来领配置；**两台真容器**实测互不串、两边日志里 key 出现 **0** 次） | ⚠️ **`SO_PEERCRED` 那条路实测走不通**（纯 Node 拿不到，见 `34-CONTAINER.md` §10.1） ⇒ 改用「**挂载表就是边界**」（§10.2）· ⚠️ 服务侧"按人起容器"还没接（②-4b，要 ②-6 的 `sudo`） |
| 5 | ✅ **预建池做完了**（②-6）：`scripts/create-tenant-pool.sh`（卷/linger/镜像进各自存储/通道目录/**systemd 用户单元**）⇒ **服务一行特权都不需要**，容器由租户自己的 systemd 看着。⚠️ **`SO_PEERCRED` 那条实测走不通**（纯 Node 拿不到）⇒ 身份改用「**挂载表就是边界**」。**两台真容器跑起来了**，端到端（网页 API → 内存 → 通道 → 容器 tmpfs）验过，见 `34-CONTAINER.md` §十一 | ✅ 已做 |
