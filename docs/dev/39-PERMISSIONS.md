...
| 4 | 🟡 **`dsh` 进镜像 ✅ 做完了**（②-1，2026-09-21：镜像 130→385MB，容器里 `dsh --version` = `0.1.5-rc.1`、空 `DSH_HOME` 能起 sdk、人格层挂得上、服务照常 200 —— 见 `34-CONTAINER.md` §七） · **换手到 uid 1000 ✅ 做完了**（②-2：`HUPO_AGENT_UID/GID` + spawn 降权 + `/data/dsh` 归 agent + 横幅如实报身份；7 条容器判据含一对正负对照，见 `34-CONTAINER.md` §八） · **root 小代理持 key ⬜ 还没** | 代理 + UDS 要写 |
