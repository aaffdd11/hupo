...
#### 5.4.2 ⚠️ 由此浮出来的：`--read-only` 那一条**理由已经没了**

`39` 原来（以及第三位）反对 `--read-only` 的理由只有一句：*"apt 必须写 `/usr`、`/var/lib/dpkg`"*。
**① 定了之后 agent 不再 apt** ⇒ **这个理由不成立了**。而 `--read-only` 恰好是**治"持久化"最直的一刀**
（P1 要挡的正是"改动变成下次开机自动读"）⇒ ③ 那道题**形状变了**：
不再是"根只读 vs apt 能装"，而是"**根只读 + `/tmp` 用 tmpfs + 用户级前缀放 `/data`**"够不够。

⚠️ **这条我不替主人定**（③ 还在 §七），只如实说：**它现在明显更便宜了**。
...
| # | 问题 | 选项 | 备注 |
|---|---|---|---|
| ① | ~~"agent 能装包" vs "agent 读不到自己的 key"~~ | ✅ **已定：甲** | 见 §7.1；**由此推翻了 dpkg 那组能力，并让 ③ 的形状变了** |
| ② | **`V4` 字面**（`grep -rl "sk-" /data /home` = 0）与 **"key 在盒里"** 直接冲突 | **重划范围**（要你签）· 走代理 | ⚠️ **不许换路径骗过**（挂到 `/data`、`/home` 之外就说"过了"）· ⚠️ ① 定成甲之后**冲突没消失**：key 仍在盒内，只是**agent 够不着** ⇒ 判据该从"盒里没有"改成"**盒里只有 root 够得着**" |
| ③ | **rootfs 要不要可写** | 要（apt 能装）· 不要（只读根 + `/tmp` tmpfs） | ⚠️ **① 定了之后这条的形状变了**：原来"要可写"的唯一理由是 apt，**那理由没了** ⇒ 只读根现在明显更便宜（见 §5.4.2）。⚠️ 但只读根**不等于**治住持久化：挂进去的那几份要靠挂载表 `strict` |
| ④ | **盒内 agent 的 uid 是否固定** | 固定（定卷属主与 N21 路径）· 不固定 | 不固定 ⇒ N21 的路径会漂。⚠️ ① 定成甲之后这条**基本没得选**：`/run/hupo/creds.yaml` 的"agent 够不着"**就靠 1000 vs 0 这个差** ⇒ 建议**固定 1000**（这也是镜像里已经写死的那一个） |
