...
| 4 | 🟡 **`dsh` 进镜像 ✅ 做完了**（②-1，2026-09-21：镜像 130→385MB，容器里 `dsh --version` = `0.1.5-rc.1`、空 `DSH_HOME` 能起 sdk、人格层挂得上、服务照常 200 —— 见 `34-CONTAINER.md` §七） · **换手到 uid 1000 ✅ 做完了**（②-2：`HUPO_AGENT_UID/GID` + spawn 降权 + `/data/dsh` 归 agent + 横幅如实报身份；7 条容器判据含一对正负对照，见 `34-CONTAINER.md` §八） · **root 小代理持 key ✅ 做完了**（②-3：`src/model-proxy.mjs` + 静态 patch 改 `baseURL`/`apiKeyEnv`；容器里实测：agent 经代理 **200**、自己读 key 文件 **`EACCES`**、上游收到的是**真 key 而不是占位符**、V4b 命中 **0**；9 条单测 + **四处变异验证**，见 `34-CONTAINER.md` §九） | ⚠️ 只剩「**key 从外面注入**」那一步（②-4） |
