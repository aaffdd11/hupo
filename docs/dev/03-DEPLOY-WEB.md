...
>
> **打开：<https://w.stalkerai.cn>** —— 密码**不在仓库里**（问主人）。
>
> 🔴 **2026-09-21：这里原先写着口令本身，已经删掉。** 那一版进过 git 历史并推到了 GitHub
> ⇒ 按 `AGENTS.md` §六 第 1 条（**不许把密钥写进任何会显示给别人的地方**），
> 那个口令**必须换掉**——删掉这一行只能止住"继续传播"，止不住已经发生的那次。
> 换完再把这里改成新口令的**存放位置**（例如"问主人"），**不要**写口令本身。
