...
## 附二：**为什么这一栏没有「改一条」和「插话」**（2026-09-26 取证，别再当成"还没做"）

DSH 的 QueueDock 一行上有 编辑 / 删除 / 插话 三个动作；我们只做了"删除（不发了）"。另外两个**不是没做，是各有硬原因**：

| 动作 | 结论 | 依据 |
|---|---|---|
| **插话（Steer）** | 🔴 **我们今天这条协议给不了** | 我们 L2↔L3 走的是 **SDK stdio JSON-RPC**：`dsh-sdk-jsonrpc-server` 那一侧**只有 `session/prompt`** 这一条会话方法（本机实测：`grep -rho "session/[a-z-]*" .../dsh-sdk*/lib/*.js` ⇒ 只有 `session/created` / `session/event` / `session/prompt`）。**没有任何 inbox / steer 入口**。而 DSH 自己的窗口能插话，是因为它连的是**另一套**（web 档的 `dsh-api-session-controller`：`session/prompt {mode:'queue'\|'steer'}`）。⇒ 要"插话"，得把我们 L2↔L3 从 SDK 档换成 web 那套 API —— 那是一次**架构级**改动（`115` §六·乙），不是给队列条加个按钮 |
| **改一条（Edit）** | 🔴 **今天做会立刻让页面说假话** | 我们这一侧，"这句话已投递"在**投递那一刻**就 `user/echo` 落进了可见日志（`say.js`）⇒ 屏幕上已经写着原文。**改队列里那张票的文字**，改的是**将来投给 agent 的内容**，屏幕上那句**不会跟着变** ⇒ 他看到的和它做到的**不是同一句**。要能做，得先有"撤回那一条回声"的协议（今天没有）。⇒ 替代做法已够用：**撤掉，重打一遍**（`不发了` 就在那儿） |

⚠️ 这两条**不是"以后再说"**（手册 §2.1 纪律 4）：**在协议换到 web 那套之前，这一栏就只有"排队看得见 ＋ 撤得掉"**。
