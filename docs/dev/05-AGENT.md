...
> ⚠️⚠️ **v1.1 补一句（`07-TIMEOUT.md` §六）**：光带 `bootId` **不够**。
> 会话记录**落在 `$DSH_HOME/sessions/` 里、跨进程活着**，所以
> **同一个进程里换一个 agent 实例也必须换 id**——
> 否则超时硬收口卸掉再起时，真 DSH 报 `already exists`，
> **用户接下来每一句都发不出去**。现在的 id 是三级：
> `会话名.bootId.第几个实例`。
...
| # | 没做 | 坑在哪 |
|---|---|---|
| 1 | ~~**跨重启接记忆**~~ | ✅ **已做完**——见 `06-RECAP.md` |
| 2 | **D7 的过程可见性** | `step/*` 事件已经在手上了，**但要先有"人话表"**，否则推出去就是内部词 |
| 3 | ~~**超时收口**~~ | ✅ **已做完**——见 `07-TIMEOUT.md` |
| 4 | **对账续做** | 开机扫未完成的任务。**下一批最要紧的一条** |
| 5 | **多作用域** | 现在只有一条时间线 |
| 6 | **推理原文那一档** | 事件里有，但**故意不推**——它是唯一有泄露风险的字段 |

## 七、下一步

**对账续做**（手册 D5.9「被打断的活会自己接着做」）。

`07-TIMEOUT.md` 已经把"卡住的一轮"收干净了——但它只做到
**"告诉它断了"**，没做到**"接着做"**。续做要能回答"上一轮做到哪儿了"，
所以它前面还欠一步：**把超过 15 秒的一轮挪成一个独立任务**（手册 §10.2）。

> 📌 **跨重启接记忆在 `06-RECAP.md`、超时硬收口在 `07-TIMEOUT.md`**（实测：公网重启后还记得上一句）。
> 那一批顺手修掉了一个真坑——`dispatcher` 会攥着**已经死掉的** agent 实例不放，
> 连"背景喂过没有"都因此判错。**动这一块之前先读 `06-RECAP.md` §4.4。**
