...
| **配置页「语音」那颗「试一下」（录音 → 转文字 → 一个文本框）· 为什么它不另存钥匙 · 每一种失败说哪句话 · 配置页里还有哪几处名不副实** | [`dev/123-VOICE-TEST-BUTTON.md`](dev/123-VOICE-TEST-BUTTON.md)（契约：形状与那几档字 · **钥匙只有一条路**（走 `/api/asr`，服务端按验过签的身份现取） · 一场失败一句话 · 判据 · 如实说 · **§六「名不副实（待主人定）」**） | 动「配置页 / 语音那一屏 / 那颗测试按钮 / 视频那一栏 / 钥匙生效不生效」之前 |
