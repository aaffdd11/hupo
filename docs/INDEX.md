...
| **聊天窗口重做（第三批：轨迹那一屏）—— 🔴 2026-09-26 已按主人决定删掉** | [`dev/118-TRAJECTORY-VIEW.md`](dev/118-TRAJECTORY-VIEW.md)（**这一屏今天在仓库里没有了**；那一份留作记录：顶上那条横幅写明删了什么 / 留了什么，后面是当时那两个 tab 与"切回来滚动位置不动" · 那张表的纯逻辑与判据 · **我们收不到、所以一格都不画的那几样**） | **只在问"为什么以前有聊天/轨迹两档、当时它长什么样"时读**；它**不再是**改代码的依据 |
...
| **配置页「语音」那颗「试一下」（录音 → 转文字 → 一个文本框）· 为什么它不另存钥匙 · 每一种失败说哪句话 · 配置页里还有哪几处名不副实 · 它在真页面上跑过一遍的读数** | [`dev/123-VOICE-TEST-BUTTON.md`](dev/123-VOICE-TEST-BUTTON.md)（契约：形状与那几档字 · **钥匙只有一条路**（走 `/api/asr`，服务端按验过签的身份现取） · 一场失败一句话 · 判据 · 如实说 · **§六「名不副实（待主人定）」** · **§九 真浏览器取证（假麦克风 ＋ 真上游：三步逐条读数 ＋ 四张原图）**） | 动「配置页 / 语音那一屏 / 那颗测试按钮 / 视频那一栏 / 钥匙生效不生效」之前 |
| **主人报的那三条触屏症状（滑不动 / 按钮点不开 / 收起才能发送）· 为什么"瞬点判据"看不见它们 · 长按那一层为什么会把整个窗口吃掉 · 哪一条没结** | [`dev/124-TOUCH-REGRESSION.md`](dev/124-TOUCH-REGRESSION.md)（触屏回归的账：**手按住不放 ⇒ 弹一层带 barrier 的底部单子 ⇒ 时间线、抓手行按钮、输入条一起失效**（而收起那一档没有气泡可长按 ⇒ 只有它正常）· 复现装置与读数（Android UA ＋ 合成触控 ＋ 假 `visualViewport` 装键盘）· **没复现的两条如实说** · 一条**没结**的账（窄屏下那一排动作排在视口外、无障碍树里的矩形挂在 tab 的坐标上）· 同一批那个 `Spacer` 的读数在 §三·二） | 动「气泡长按 / 浮窗标题行那一排动作 / 任何 `showModalBottomSheet` 弹层」之前 |
| **租户那台「填了语音那三样却从来没送进盒子」· 为什么按钮"自己就停下来了" · 推送的触发与盒子里的读法各缺什么 · 还卡在谁签字那一步** | [`dev/125-TENANT-VOICE-CREDS.md`](dev/125-TENANT-VOICE-CREDS.md)（两处根因：**① `setCreds` 只在带 `model` 的那一拍才推**（已修已部署）· **② `credsFor` 没收到 `env` ⇒ 盒子里读单文件那条兜底路从没走过**（已修、**要发布产品层**）· 修前/修后读数 · 今天能录的两条路） | 动「`/api/creds` / 租户盒子那条通道 / 语音或图片那几屏的边界话 / 发布产品层」之前 |
| **小程序那条顶栏与返回箭头撤掉之后：出口在哪 · 那颗 home 多大 · 进屏那条浮窗停留多久 · 撤顶栏顺手修掉的那个时序缺陷** | [`dev/126-MINIAPP-NO-HEADER-HOME.md`](dev/126-MINIAPP-NO-HEADER-HOME.md)（顶栏整条撤掉（不是藏起来）· home = 回桌面（关屏 ＋ 收聊天 ＋ 回主线）· 那条浮窗的六条规矩 · 🔴 顺手修掉"收起那一帧的内缩还是展开时的高度 ⇒ 页面被压扁、`TabBar` 溢出"） | 动「小程序容器 / 那颗 home / 输入条那一条 / 内置三屏的抬头」之前 |
