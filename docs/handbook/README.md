...
> 🧭 **不知道读哪一份 ⇒ 先看 [`../INDEX.md`](../INDEX.md)**（L0 路由：问题 → 文件 §节；
> 它**只放指针、不放结论**，所以它不会过期）。
...
| **实现** | 🚧 **进行中**（**条数/批次/欠账只住在 [`../dev/00-PROGRESS.md`](../dev/00-PROGRESS.md)**，这里不抄） |
| **线上** | ✅ **<https://w.stalkerai.cn>** —— 登录 + 聊天 + 会答话 + 重启还记得 + 卡住会收场 + 看得见它在做 |
| **本机是什么** | ⚠️ **就是跑线上服务的那一台**（`127.0.0.1:8020` + frp 隧道）。**开发与线上同机** |

> 🔴 **2026-09-22 更正**（这里原来写着"**本机不是生产机**……`AGENTS.md` 描述的是另一台"）。
> **那句话已经是假话**，而且它最坑人：照它做会**在本机到处找服务**，或者**以为线上在别处、改完不管部署**。
> 现在的事实：本机 `v2/services/core/src/serve.js` 听 `127.0.0.1:8020`，对外是 `https://w.stalkerai.cn`，
> 两台真租户在跑。另一台（`hupo.stalkerai.cn`、systemd `concierge-core`）是**上一代**那个部署。
> ⇒ **要问什么读哪一份，先看 [`../INDEX.md`](../INDEX.md)**（L0 路由）。
...
| **3** | **纯函数进 `test/unit`，不写界面断言。** `test/widget` 一般是提示档，**但 `accessibility_test.dart` 是硬闸**（D3.5 点名）。新验收写进 `test/unit` 才算有人守。 |
...
| **6** | **文档分层，指针要过闸。** 层与层之间**只许引用、不许复述**（一条事实只有一个家）。跑 `node scripts/check-docs.mjs`：链接可达 + `#锚点`可解析 + 路由层不许出现数值。**它挂在提交前**，红了就推不出去。 |
