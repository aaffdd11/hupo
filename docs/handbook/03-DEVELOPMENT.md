...
| **`step/summary`** | 新事件 | `{text}` | 调度器 | D7 默认档：一句人话。**必须是人话，不许出现内部名** |
| **`step/detail`** | 新事件 | `{tool, target, ms, ok}` | 调度器 | D7 第三档：步骤流水 |
| **`step/reasoning`** | 新事件 | `{text}` | 调度器 | D7 第四档：**只给主人，默认关** |
...
⚠️ `DSH_HOME` **当前无处设置**（只在 docs 与 `scripts/diagrams.mjs:585` 出现过）。
