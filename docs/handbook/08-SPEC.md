...
| **页面切换**（首页 ⇄ 登录页） | **320ms easeOutCubic**（淡入淡出 + 新屏轻轻上浮） | 🔴 **2026-09-22 主人要的**：*"我希望是一个丝滑的过渡展示效果，而不是突然出现的效果。"* ⚠️ 硬切 = 用户看到"闪了一下"；时长住在 `models/design.dart` 的 `motionPage`，判据在 `test/widget/landing_transition_test.dart` |
