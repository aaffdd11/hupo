...
| `/api/app-remove` | POST | 需令牌 | **从桌面上删掉一个小程序**（主人 2026-09-25）。body `{id}` ⇒ `{ok:true}`。🔴 **以盒子为准**（同 B15/B17）：租户这一下**在他盒子里落**（宿主经现有隧道请盒子动它自己的库）；**盒子不通 ⇒ 如实 503**，**绝不**去动宿主那份。⚠️ 它是**软删**（`Apps.remove()` 挪进 `.removed/` ＋ 审计），但**今天没有"拿回来"的入口** ⇒ 界面上**不许**说"还能拿回来"。认不出 / 不在他这儿 ⇒ **404 ＋ 一句人话**。契约 `docs/dev/103-APP-DELETE.md` |
