...
>
> 评审另外挖出**两个今天就在咬人的 bug**（§1.4 与 §4.3），它们**不属于文档错误**，是代码里的坑。
>
> **评审报的 7 条 + 我随后浮出的 P-l 共 8 条，已全部拍板（全取甲）**，见 §8.2。
> ⇒ **批 0 整个关闭**，批 1 可以开工。**批 0-b（旧客户端兼容）因 P-l 而不必再做**（§8.3）。
...
> 第 2、3 条已由 **P-e 拍板（甲：拆 `Connection` + 每会话一个 `Session`）**解开。
> ⚠️ **但批 0-a 还剩一半（P-l：统一时间线的编号从哪来）没答** ⇒ 动 §4.1 之前先看 §8.3。
...
> 下表**只留站得住的**；站不住的三条已由 P-h / P-i 拍板处置（见下）。
...
| 字段 | 为什么删 |
|---|---|
| **`source`**（我原写 `user` / `dispatcher` / `agent:<scopeId>`） | ❌ **概念错**：它**与 `agent` 重复一半、与 `origin` 混淆一半**。现网已经有**两条轴**：`origin`（`dispatcher.js:549-556` `nextProvenance()`：有 pending 用户消息 = `reactive`，否则 `proactive`）回答"**为什么开口**"；`agent`（`conversation.js:193` 发 `'agent'｜'dispatcher'`，**已经在线上、只是客户端不读**）回答"**谁在说**"。而 `message/start` **永远是助手在说**——`source` 里的 `user` 想表达的其实是 `origin` 的地盘。⇒ **✅ 已拍板（P-i 甲）：删掉 `source`，用回 `agent` + `origin`**。客户端的活儿变成「**把已经在线上的 `agent` 读起来**」 |
| **`catchUp`** | ✅ **已拍板（P-h 甲）：保留为线上字段**。⚠️ 两条硬要求：**不许 `Object.assign(事件, {catchUp})`**（事件对象来自 `conv.log`，**会被写坏**），**必须浅拷贝**；**服务端只在 `sinceSeq > 0` 时标 `true`** —— 因为 `main.dart:79` 的 `connect()` **默认 `resumeFrom = 0`**，**首次打开也走 replay**，无条件标会把**全量历史**标成"断线补发" |

### 2.3 三条语义规则（**必须写进 PROTOCOL.md**）—— 评审逐条改过

| # | 规则（**改后**） |
|---|---|
| **P-1** | ⚠️ **改口径，且必须分「现状」与「目标」**：**现状** `seq` 是**每 `conversationId` 一份**（`conversation.js:14`、文件头 `:3`）⇒ 旧措辞"全局 seq"是错的；**目标 = 每个用户那条可见时间线一个计数器**（X1.1/X1.2 + **P-l 甲：一条日志**），由调度器统一发、= 那条日志已落盘最大 + 1。⇒ **要写死的是「持久事件的 `seq` vs 续传游标 `sinceSeq`」**——客户端有**两个游标**：传输层 `_sinceSeq`（`websocket_transport.dart:59/206`）与 UI 层 `lastSeq`（`chat_controller.dart:52/249`），**协议里两个名字分开写，不许混成一个词** |
...
| **P-3** | ⚠️ **重写**（原文自相矛盾）：原文说"瞬态事件不占号"又说"`timeline/marker` 占一个位置"。**裁决定稿**：**"不占号" = "不上时间线"**。⇒ **控制帧与 UI 状态不算时间线事件**：`client/reload` 是控制帧、本地应声是 UI 状态，二者**在进入 `timeline` 之前就被消费掉**；⇒ **`marker` 是持久事件，取号、落盘、有位置**。⇒ 好处：**`ServerEvent.seq` 可以保持 `required int`**，`test/unit/timeline_test.dart:128`（"缺 seq 被拒"）**继续绿**，**不用动基类、不用破硬闸**（**✅ P-g 已拍板取甲**） |
...
**定稿**（⚠️ 评审说"与按可见时间线各数各的如何共存**必须写死**"——**已由 P-l 拍板（甲）写死**）：
...
2. **协议字段一旦上线就冻结。**
   ...
   评审砍掉了 `source`（**P-i 甲：删**）并质疑了 `catchUp`（**P-h 甲：保留，但只在 `sinceSeq > 0` 时标**），
   因为**它们当时没有数据源或语义重叠**。
...
### 8.2 已拍板（**2026-09-20 主人确认，全部取甲**）

| # | 定稿 | 落在哪 |
|---|---|---|
| **P-e** | **拆 `Connection`（顶层持有）+ 每会话一个 `Session`** | §1.4 欠账 2/3、§四 淘汰 |
| **P-f** | **`this.turns` 改用 `info.turn`**（五处） | §4.3 |
| **P-g** | **"不占号" = "不上时间线"**；marker 是持久事件；不动基类 | §2.3 P-3、§4.1 |
| **P-h** | **`catchUp` 加为线上字段**，只在 `sinceSeq > 0` 时标 | §2.2、§2.4 |
| **P-i** | **删掉 `source`**，用 `agent` + `origin` | §2.2 |
| **P-j** | env **先做减法**（只删密钥类），严格白名单随后 | §4.4 |
| **P-k** | `emit` 统一包一层 + 进程级 notice + 有界退出 + 退避重启 | §4.2 |
| **P-l** ⭐ | **一条可见时间线 = 一条日志**；作用域只是事件上的 `scopeId` 标签；号 = **那条日志**已落盘最大 + 1 | §4.1、§2.3 P-1 |

### 8.3 已无待裁决 —— **批 0 整个关闭**

> ✅ **P-l 已拍板（甲）** ⇒ **批 0-a 全部关闭**（P-e 定连接与状态，P-l 定编号与排序）。
> ✅ **批 0-b（旧客户端兼容）不必再做**：`seq` 对现网客户端的语义没变（§4.1）。
> ✅ **§4.1 的措辞已按 P-l 统一**（"该会话" → "该用户那条可见时间线"）。
> ⇒ **批 1 可以开工。**

**P-l 留下的唯一小选择**（**不阻塞批 1**，到批 3 碰 `conversation_list_screen` 时再定）：
"会话"内置应用改成作用域清单后**与桌面图标墙是同一份数据**，可能重复了。
倾向：**桌面图标墙 = 手机第一入口**；"会话"退为**宽带屏左栏**（仍是同一份数据），**不再单独做列表页**。
