...
>
> **评审报的 7 条待裁决已全部拍板（P-e…P-k，全取甲）**，见 §8.2。
> **还剩 1 条没答：P-l（统一时间线的编号从哪来）** —— 它是批 0-a 剩下的一半，见 §8.3。
> ⚠️ **P-l 定之前，§4.1 的措辞只是"把现状收口"，不等于 X1.2 的目标。**
...
| # | 待裁决 | 为什么现在必须定 | 推荐 |
|---|---|---|---|
| **P-l** | **统一时间线的编号从哪来**（**批 0-a 剩下的另一半**） | P-e 定了"每会话一个 `Session`"、P-g 定了"`seq` 是持久的"，但**没人回答"多条作用域拼在一起按什么排序"**；而 **X1.1/X1.2 已拍板"调度器统一发号 + 已落盘最大 + 1"** ⇒ 与现状（每会话一份）目标不同；§一 那句"如何共存"**至今没写死** | **甲：一条可见时间线 = 一条日志**（作用域只是事件上的 `scopeId` 标签）。⭐ 它**很可能连带解掉批 0-b**（旧客户端语义不变 ⇒ 不会屏幕全白） |

> ⚠️ **P-l 定之前**：`R2-ENGINEERING.md` §4.1 的"持久事件的号 = **该会话**已落盘最大 + 1"
> **只是把现状收口，不等于 X1.2 的目标**。两者在 P-l 定完之后要统一措辞。
