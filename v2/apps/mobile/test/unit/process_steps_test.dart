// 步骤那一路（`step/*`）+ 推理原文（手册 **D7** / 契约 `docs/dev/122-TWO-PROCESS-LEVELS.md` §三·§四）。
//
// ⚠️ **`step/*` 今天是老客户端 `steps` 档的通道**（那一档已从菜单砍掉，
//    界面不画它，见契约 §一）。这一份仍然钉它，是因为**协议不许破**：
//    老客户端还在发 `?level=steps`，服务端也还按契约发；状态机这一层照旧要
//    乱序保护与收口清理。**收得到 ≠ 画出来**（画不画是界面那一层的事）。
//
// ⚠️ 契约点名要的两条**都在这一份里**：
