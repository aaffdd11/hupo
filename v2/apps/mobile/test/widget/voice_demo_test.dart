...
void main() {
  ...
  testWidgets('🔴 点话筒 ⇒ 语音档：有"按住 说话"，而且**明标「演示」+ 一句实话**', (tester) async {
    ...
    expect(find.text(voiceNotWired), findsOneWidget, reason: '★ 那句实话必须在（还没真的开麦）');
  });

  testWidgets('🔴 按住 ⇒ 出字（"实时转文字"的形状）；松手 ⇒ **进打字框、不自动发送**', (tester) async {
    var sent = 0;
    await tester.pumpWidget(
      MaterialApp(home: Scaffold(body: Composer(onSend: (_) => sent += 1))),
    );
    await tester.pump();
    await tester.tap(find.byTooltip(voiceToMic));
    await tester.pump();
    ...
    final gesture = await tester.startGesture(tester.getCenter(find.text(voiceHoldToTalk)));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 700));
    expect(find.text(voiceReleaseToSend), findsOneWidget, reason: '按着的时候该说"松手 发送"');
