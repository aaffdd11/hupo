...
/// **浮窗该不该为这一条通知喊一声？**（契约 `docs/dev/29-NOTICE.md` 约束 1）
///
/// 两条都不喊：
///   · `catchUp` —— 服务端标的"补发"（你不在的时候发生的事）；
///   · `readingHistory` —— **这条连接还在读首屏那段历史**。
///
/// 🔴 **第二项是 2026-09-22 补的，起因是主人报的一个真现象**：
/// *"登录后，出现'刚才出了点事，我已经重来了'。但是其他手机的并没有出现这段话。"*
/// 查下来是**两边各说各话**：
///   · 服务端（决策 P-h）说：`sinceSeq == 0`（第一次打开）是**历史本身**，不是补发
///     ⇒ **故意不打** `catchUp`（`resume.js` 那一段注释写得很清楚）；
///   · 客户端却把"没打 `catchUp`"直接当成"现在发生的" ⇒ **冷启动整段历史都被喊了一遍**。
/// ⇒ 于是那条**很久以前**的崩溃通知，在他**每一次登录**时都弹一次浮窗。
///    那既是骚扰（R1.2 通知疲劳），也是**假话**（"刚才出了点事"其实不是刚才）。
///
/// ⚠️ **判据的落点就是它**：历史（无论"你不在时"还是"首屏"）**只进时间线、不喊浮窗**；
///    只有连上之后**新来的**才喊。时间线那一条照样留着 —— 通知要经得起"你不在"。
bool shouldPopNotice({required bool catchUp, required bool readingHistory}) =>
    !catchUp && !readingHistory;
