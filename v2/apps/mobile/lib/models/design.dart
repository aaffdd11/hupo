...
///   · **平面卡片**（设置行、通知、导出框…）= 白底 + [radiusCard]/[radiusField] + `line` 细描边，**不画阴影**；
///   · **浮起来的**（聊天浮窗、图标格、小程序扩开那一层）= 白底 + 圆角 + **阴影**（`tileShadow*`），**不描边**。
///
/// 为什么值得写下来：混着用的后果不是"难看"，而是**分不清哪一层在上面**
/// —— 用户看到的层次感是我们用这两样东西"画"出来的，规则一乱就没有层次了。
/// 判据（棘轮）：`test/unit/design_tokens_test.dart`（写死的圆角只许少不许增）。
///
/// 圆角（收成三档，别再各写各的）。
