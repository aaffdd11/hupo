...
/// **多选态里"这一条被选中了"**那层淡淡的罩色（契约 `docs/dev/106-CHAT-SELECT.md` §一）。
///
/// ⚠️ 它是**半透明的主色**，压在气泡原来的底色上（`Color.alphaBlend`）——
///    这样用户那条与它回的那条**各自都看得出被选中**，而不是只对其中一种生效。
/// ⚠️ 选中**不许只靠颜色**（色盲 / 屏幕反光）：另外两路是那一圈 `accent` 描边
///    和气泡里那个勾。
const Color selectWash = Color(0x2EC8452F);
...
/// 细间距（4）：**紧贴的两行之间**那种（说明文字贴着输入框、提示贴着状态行）。
/// ⚠️ 2026-09-24 加它是因为棘轮当场拦下了新写的一处"高 4 的占位框"
///    —— 规矩是"数值只住 design.dart"，不是"在别处写 4"。
///    （⚠️ 注释里也别把那句代码原样写出来：那条棘轮扫的是**源码文本**，
///      连注释里的样例都算一处 —— 当场被抓过一次。）
