...
const Map<String, String> forbiddenWords = {
  ...
  // 假承诺
  // ★ 2026-09-24（P0-5）**收窄过一次**：原来是一刀切禁掉，理由是当年"它根本没在听"。
  //   现在**真的在听**了（真开麦 ASR，契约 `71-MIC-ASR.md`）⇒ 按 D5.13 的原意收窄成：
  //   **只在真的在录音时才可以**（D5.13：*"不录音时禁用「听」字"*）。
  //   ⚠️ 唯一的合法落点是 `hearing_words.dart` 的 `hearListening`（只在 `Hearing.listening` 为真时画）；
  //      别处一律仍然禁 —— 判据 `test/unit/forbidden_words_test.dart` 扫的是**整棵树里的字符串字面量**。
  '正在听': '只在**真的在录音**时才可以（D5.13）：唯一合法落点是 hearing_words.dart 的 hearListening',
