...
//
// ── 批 3：地址上多带一个 `level`（过程档位）──────────────────
//
// `level` 是**连接级**的：服务端按每条连接决定发多少过程
// （`docs/dev/122-TWO-PROCESS-LEVELS.md` §三）。⇒ 它必须跟着地址走，
// **不能**连上之后再补发一个"改档"的帧（那会变成全局开关）。
// 契约：不带 = `doing`；客户端**永远带上**，好让"用户在哪个档"这件事
// 在服务端和客户端只有一种理解。
//
// ⚠️ **四个 wire token 一个都不许改**（`quiet|doing|steps|reasoning`，手册纪律 2）：
//    新客户端只发 `doing` / `reasoning` 两个（另两个入口已砍，见契约 §一），
//    但老客户端还在发 `quiet` / `steps` —— 这一条只多不少。
//
// ── 批 4：地址上再多带一个 `scope`（一个图标 = 一条对话）──────────
//
// 契约 `docs/dev/83-APP-WORKSPACE.md` §六·4（§五 定的是**甲**：跟着图标走）：
// 打开某个"我的小程序"⇒ 下面那条聊天就是**它的**对话。
//
// ★ **C 期更正**（契约 `docs/dev/84-DISPATCHER-FOCUS.md` §三·3/§四）：
//   `scope` **不再是"这条连接只听这一间"** —— 手册 §四 核心原则 3 说
//   "**一个 WS 连接是物理约束**，不能每个会话一条连接"。
//   ⇒ 地址上这个 `scope` 现在的意思是"**初始焦点**"：这条连接一上来先看哪一间。
//     之后切房间**不重连**，走 `services/stream.dart` 的 `focus()`（客户端→服务端一帧）。
//   ⚠️ 它仍然**在地址上**（老服务端只认它；老客户端也只带它）——这一条不破。
//
...
/// * [level] 过程两档（契约 `docs/dev/122` §三）。默认 [defaultProcessLevel] = `doing`。
/// * [scope] **初始焦点**（契约 84 §三·3）：这条连接一上来先看哪一间。
///   默认 [mainScope] = 桌面上那条主对话。
///   ⚠️ 之后切焦点**不重连**（走 `stream.dart` 的 `focus()`）——地址上这个参数
///      仍然带着老语义（老服务端只认它），但它不再是"这条连接只听这一间"。
///
