// **DSH 那一套设计 token → Flutter 的那一小层换算**。
//
// 为什么单独一份：`models/dsh_design.dart` 是纯逻辑层（不 import `material`），
// 所以"字号 + 绝对行高 → `TextStyle`"这种事只能住在 `widgets/` 这一层。
// 而它**每一个用 token 的控件都要**（工具行、右栏、排队条、输入条……）——
// 各写一份就是几份会漂的换算（本项目第一条纪律：同一份数字不许有两处）。
//
// ⚠️ 这一层是 2026-09-26 从 `widgets/tool_row_view.dart` 抽出来的（那会儿
//    轨迹那一屏与两个 tab 也要用同一份换算）。轨迹那一屏当天按主人决定砍了
//    （见 `docs/dev/118-TRAJECTORY-VIEW.md` 顶上那条横幅），这一层留着 ——
//    今天它服务的是工具行 / 右栏 / 排队条 / 输入条 / 气泡。
//
