...
test('★ 公开路由只有这几条（加任何一个都要问一句"它真的必须公开吗"）', () => {
  // 🔴 2026-09-24（P1-11）**从三条变成四条**：`/api/send-code` 一直是真实可达的
  //   （那道分支在鉴权之前 —— 登录**前**就要用它发码，那时根本没有令牌），
  //   而这张常量名单里**漏了它** ⇒ 常量在说假话。
  // ⚠️ 这条判据的用意没变：**每加一条都要能说清"为什么它必须公开"**。
  assert.deepEqual([...PUBLIC_ROUTES], [
    '/api/version', // 版本/指纹：登录页也要显示
    '/api/auth', // "这台设了口令没"：登录页第一步
    '/api/login', // 登录本身（要口令/验证码）
    '/api/send-code', // 发登录验证码：令牌还不存在，所以必然公开
  ]);
});
