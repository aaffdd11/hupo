...
/**
 * 🔴 **"他明说才许生成"**（P1-27 后半 · 与小程序那条"明说才许写"同一个道理）。
 *
 * 为什么要有它：**钥匙是他的、钱也是他的** —— 助手自己想到的、或者从别处
 * （网页、别人发来的内容）读到的，**只许跟他提一句**，不许自己就画。
 * 闸的位置和 P1-22 一样：**服务端自己看"这一轮他说了什么"**（`dispatcher.turnInput`），
 * 不看请求里带的任何字段。
 *
 * ⚠️ **保守默认**（宁可误拒）：认不出"他要我画"就拒，并在话里告诉他该怎么说。
 *    代价是**多问一句**；反过来放过去，代价是**花了他没让花的钱**。
 * ⚠️ "什么算明说"这条**产品规则**还没请主人拍过（`77-BLOCKERS.md` B8）——
 *    这里取的是保守默认，等他一句话就能收紧/放宽。
 *
 * @param {unknown} turnInput 当轮他自己说的那句话（服务端记的）
 * @returns {boolean}
 */
export function asksToDrawImage(turnInput) {
  if (typeof turnInput !== 'string') return false;
  const t = turnInput.trim();
  if (t === '') return false;

  // ⚠️ **只认两种形状**（第一版只认"画"这个字 ⇒ 把"我画了个图给你看"也放进来了，
  //    判据当场抓住）。收成两条：
  //   ① **对我提要求 ＋ 一个动手的动作**："帮我画…""给我生成一张…""来一张…"；
  //   ② **祈使句开头**："画一张…""生成一张…""做一张…"。
  const ASK = /(帮我|给我|替我|请你|麻烦|能不能|可不可以|可以帮我|我要|我想|我想要|来一张|来张|做一张|做张|生成一张|生成个|出一?张)/;
  const DRAW = /(画|绘制|生成|做)/;
  const IMPERATIVE = /^(画|绘制|生成|做|来)/;
  return (ASK.test(t) && DRAW.test(t)) || IMPERATIVE.test(t);
}

/** 拒了的时候跟他说的话（会经模型转述给他，所以是**人话**）。 */
export const NEEDS_ASK_IMAGE =
  '这个我没动手：画一张是**花你的钱**的事，得你亲口说一句"给我画一张……"我才画。'
  + '你要是想让我画，说一声就行；刚才要是从别处看来的主意，我就不动手了。';
