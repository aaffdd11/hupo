...
//      契约 `docs/dev/122-TWO-PROCESS-LEVELS.md` §二）：
...
//      ★ **2026-09-26**（主人「名不副实的要去掉。」）：客户端菜单收成两档
//         （`doing` / `reasoning`），新客户端**不再发** `steps`。
//         ⇒ 🔴 **`step/*` 这条通道今天只为老客户端的 `steps` 档存在**：
//           新客户端收得到（`reasoning` 那一阶也放行它）但**不再消费**
//           （界面不画，见 `docs/dev/122` §四）。**这一层的语义一个字不改**。
//
//   ⑤ ⚠️ **步骤的"类别"由随后的 `tool/call` 补上，而且只补不发名字。**
//      `step/start` 里没有工具名（实测），所以只靠它，老 `steps` 档会是一串
//      一模一样的兜底句。契约 `122-TWO-PROCESS-LEVELS.md` §6.1 的裁决是
...
/**
 * 工具名 → **步骤类别**（契约 `122-TWO-PROCESS-LEVELS.md` §6.1 的裁决）。
 *
 * 为什么需要它：DSH 的 `step/start` 里**没有工具名**（工具名在随后的
 * `tool/call` 上，实测）⇒ 只靠 `step/start`，老 `steps` 档会是
 * **一串一模一样的兜底句**，等于把那一档做成噪音。
 * ⚠️ 今天这条补发**只为老客户端的 `steps` 档**服务（见 `#emitStep` 顶上那条）。
 *
 * ⚠️ 三条不许破：
...
export class TurnTranslator extends EventEmitter {
  ...
  /**
   * **一步一步的过程**（D7 的"步骤流水"那一档）——瞬态，永不落盘。
   *
   * ★ **2026-09-26**（主人「名不副实的要去掉。」）：那一档已从**客户端菜单**砍掉
   *   （工具行说得更准更全、而且落盘）⇒ 🔴 **这条通道今天只为老客户端的
   *   `steps` 档存在**（`server.js` 的 `LEVEL_EXTRA_TYPES` 照旧放行它，
   *   新客户端即使收得到也不再消费/不再画，见 `docs/dev/122` §四）。
   *   **这一层的语义一个字节都不改** —— 砍的是入口，不是协议。
   *
   * 形状照契约 `122-TWO-PROCESS-LEVELS.md` §三（新增事件，**不动既有字段的语义**）：
   *
